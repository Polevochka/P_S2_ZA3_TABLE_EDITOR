...
var f: file of Stud; // файл студентов
    ...
    n: integer; // число прочтённых студентов файле
    summPoints: integer; // сумма баллов
    exam: string; // номер экзамена
    ave : real; // средне арифметическое оценок
    ave_str: string; // строковое представление среднего
    i: integer;
begin

  // Получаем номер экзамена от пользователя
  exam := inputbox('Ввод значений', 'Введите номер экзамена (1, 2, 3)', '1');
  ...
  n:= 0;
  summPoints := 0; // с суммой баллов то же самое
  ...
  begin
    ...
    // проверяем за какой экзамен нужно среднее арифметическое
    case exam of
      // если первый экзамен то складываем с общей суммой 1-ую оценку
      '1': summPoints := summPoints + s.o1;
      // Второй - 2-ую
      '2': summPoints := summPoints + s.o2;
      // И тд
      '3': summPoints := summPoints + s.o3;
    end;
  end;
  // Прочитали весь файл - надо его закрыть
  CloseFile(f);

  // Вычисляем среднее арифметическое всех студентов за экзамен exam(1,2,3)
  // Это сумма баллов за этот экзамен, наколичество людей получивших по нему оценку
  ave := summPoints/n;
  ...
  begin
    ...
    ColCount := 2;  // Число столбцов  для (1) названия чего выводим(пояснительное сообщение)
                    //                     (2) для вывода среднего ариметического
    RowCount := 2; // Количество строк

    // Устанавливаем ширину отдельных ячеек
    ColWidths[0] := 200;  // под сообщение
    ColWidths[1] := 140; // на средний балл

    // Заполняем заголовок нашей таблицы
    Cells[0,0] := 'Наименование';
    Cells[1,0] := 'Экзамен № '+ exam;
