...
{Является ли строка TheStart НАЧАЛОМ строки st}
function IsStartSame(TheStart: string; st: string): boolean;
begin
  // Если TheStart НАЧАЛО строки st - то pos(TheStart, st) вернёт 1
  // Если нет то pos вернёт 0

  // То что мы присваимваем имени функции и является ее
  // результатом работы
  // если pos = 1 то вернёт True - истина
  // если pos НЕ равен 1 вернёт False - ложь
  IsStartSame := (pos(TheStart, st)=1);
end;

{Является ли строка TheFinish КОНЦОМ строки st}
function IsFinishSame(TheFinish: string; st: string):boolean;
var i_s: integer; // для перебора конца строки st
    i_f: integer; // для перебора предполагаемого окончания(TheFinish)
    res: boolean; // результат работы функции
begin

  // будем обходить строки TheFinish и st c конца и проверять
  // однаковые ли у них символы
  // если попадётся хотябы один символ TheFinish - не равный
  // соответствующему символу st - то TheFinish - НЕ КОНЕЦ строки st

  // Идём с конца строк
  i_s:= length(st);
  i_f:= length(TheFinish);

  // Нам достаточно проходить TheFinish
  // То есть перебираем пока не закончится TheFinish
  // Не надо перебирать до конца символов st
  // st - скорее всего БОЬШЕ по длине TheFinish
  // и может случится так что TheFinish закончится а st ещё нет
  // тогда выйдет ошибка

  // Изначально считаем, что TheFinish - КОНЕЦ строки st
  // Если выяснится, чо это нге так, то мы позже поменяем значение
  // этой переменной
  res:= True;
  // Обходим до тех пор пока не придём к 1-ой
  // позиции строки TheFinish
  while (i_f > 0) do
  begin
    // если соответствующие символы с конца не совпадают
    if (st[i_s] <> TheFinish[i_f]) then
    begin
      // говорим, что TheFinish НЕ конец строки st
      res:= False;
      // досрочно выходим из цикла, так как нет смысла
      // дальше проверять строки
      break;
    end;
    // Переходим к следующим символам строк
    i_s:= i_s -1;
    i_f:= i_f -1;
  end;
...
// например *ab
function PodhoditMaske(maska: string; s: string): boolean;
...
begin
  ...
  begin
    ...
    // надо чтобы маска maska - была НАЧАЛОМ строки s
    // то есть
    res := IsStartSame(maska, s);
  ...
  begin
    ...
    res := IsStartSame(before, s) and isFinishSame(after, s) and (length(s) >= length(after) + length(before));
  end;
...
begin
  ...
  begin
    ...
    if (not PodhoditMaske(maska, s.Name)) then
    begin
