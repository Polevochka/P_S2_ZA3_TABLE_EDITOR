...
var surname: String; // фамилия вводимая
    groupName: String; // название группы
    o1, o2, o3: integer; // оценки
    f: file of stud;  // файл со студентами
    s: stud; // переменная студента для считывания из файла
    i: integer;
begin

  // Получаем от пользователя данные
  surname := inputbox('Ввод', 'Введите фамилию студента', '');
  groupName := inputbox('Ввод', 'Введите Группу', '');
  o1 := StrToInt(inputbox('Ввод', 'ОЦ №1', '5'));
  o2 := StrToInt(inputbox('Ввод', 'ОЦ №2', '5'));
  o3 := StrToInt(inputbox('Ввод', 'ОЦ №3', '5'));
  ...
  begin
    ...
    // Если у этого студента совпадают ФАМИЛИЯ и ГРУППА
    // с теми, что ввёл пользователь, то
    if (s.Name = Surname) and (s.Gr = groupName) then
    begin
      // Изменяем оценки студента на те, что ввёл юзер
      s.o1 := o1;
      s.o2 := o2;
      s.o3 := o3;
    end;
