...
begin
  ...
  // Хорошечно отработали -> что работа сделана
  ShowMessage('Все готово - откройте папку проекта');
...
var f: file of Stud; // Исходный файл со студентами
    s: Stud; // студент которогу будем записывать в таблицу из файла
    count2: integer; // число двоек у одного студента
    countBad: integer; // счётчик ДВОЕШНИКОВ
begin
  ...
  // Изначально считаем , что в файле нет двоешников
  // если они появятся увеличим счётчик двоешников
  countBad := 0;

  // Обходим всех студентов в ИСХОДНОМ файле
  while(not EOF(f)) do
  begin
    // считываем одного студента
    read(f, s);

    // Пока мы не знаем сколько двоек у этого студента
    // поэтому по умолчанию ставим 0
    // если двойки появятся - то увеличим счётчик двоек
    count2:= 0;

    // Находим количество двоек у этого студента
    // Здесь именно независимыми if-ами проверяем
    // ыедь оценки не зависят друг от друга
    if (s.o1=2) then count2 := count2 + 1;
    if (s.o2=2) then count2 := count2 + 1;
    if (s.o3=2) then count2 := count2 + 1;

    // Студент является отстающим если имеет от двух и более двоек
    if(count2 >=2) then
    begin
      // увеличиваем счётчик двоешников
      countBad := countBad + 1;

      // счётчик двоешников очень полезен
      // благодаря ему можно последовательно записать
      // двоешников в таблицу
      // используя индекс двоешника как номер строки
      // Причем заметтье что пропускается нулевая строка
      // ведь это строка заголовка

       // Записываем этого двоешника в таблицу
      // Чтобы слишком часто не писать StringGrid1.Cells
      // используем менаджер контекста with
      with StringGrid1 do
      begin
        Cells[0,CountBad]:= IntToStr(s.No);  // Его Номер
        Cells[1,CountBad]:= s.Name;          // Фамилия
        Cells[2,CountBad]:= s.Gr;            // Группа
        Cells[3,CountBad]:= IntToStr(s.o1);
        Cells[4,CountBad]:= IntToStr(s.o2);   {И оценки}
        Cells[5,CountBad]:= IntToStr(s.o3);
      end;
    end;
  end;
  // Прочитали ВЕСЬ ИСХОДНЫЙ файл - надо его закрыть
  ...
  // мы изменяли таблицу но сам файл не трогали
  StringGrid1.Modified:= False;
